...
class UserToolAccess(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "tool_id", "status", name="user_tool_access_user_id_tool_id_status_key"),
    )
    ...
    granted_by: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False)
    revoked_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    revoked_by: Mapped[int | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    status: Mapped[str] = mapped_column(String, default="active")

    # Relations
    user: Mapped["User"] = relationship(foreign_keys=[user_id], back_populates="tool_accesses")
    tool: Mapped["Tool"] = relationship(back_populates="accesses")
    granter: Mapped["User"] = relationship(foreign_keys=[granted_by], back_populates="granted_accesses")