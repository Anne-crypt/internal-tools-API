...
@router.get(
    "",
    response_model=ToolPaginatedResponse,
    summary="Filtrer et paginer les outils business",
)
async def get_tools(
    *,
    ...
    category: str | None = Query(
        None, alias="category", description="Nom de la catégorie (ex: Development)"
    ),
...
@router.get(
    "/{tool_id}",
    response_model=ToolDetailOut,
    summary="Obtenir le détail financier complet d'un outil",
)
...
async def create_tool(
    *, db: AsyncSession = Depends(get_db), payload: ToolCreateIn
) -> Any:
...
@router.put(
    "/{tool_id}", response_model=ToolCreateOut, summary="Modifier un outil existant"
)
async def update_tool(
    *,
    ...
    try:
        updated_tool = await tool_controller.update_existing_tool(
            db, tool_id=tool_id, tool_in=payload
        )
