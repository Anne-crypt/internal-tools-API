...
async def test_get_filtered_tools_by_department_and_status(
...
async def test_get_filtered_tools_by_cost_and_category(
    db_session: AsyncSession, seed_data
):
...
async def test_update_tool_success(db_session: AsyncSession, seed_data):
    ...
    updated = await tool_controller.update_existing_tool(
        db_session, tool_id=1, tool_in=payload
    )
...
async def test_update_tool_not_found(db_session: AsyncSession, seed_data):
    ...
    result = await tool_controller.update_existing_tool(
        db_session, tool_id=999, tool_in=payload
    )
    assert result is None
